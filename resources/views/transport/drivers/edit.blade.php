...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-foreground">Edit Driver</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="personal_number" id="personal_number" value="{{ old('personal_number', $driver->personal_number) }}" required
                            class="mt-1 block w-full rounded-md border-border bg-background text-foreground shadow-sm focus:border-primary focus:ring-primary">
                    ...
                    <div>
                        ...
                        <input type="tel" name="phone" id="phone" value="{{ old('phone', $driver->phone) }}" required
                            class="mt-1 block w-full rounded-md border-border bg-background text-foreground shadow-sm focus:border-primary focus:ring-primary">
                        @error('phone')
                            <p class="mt-1 text-sm text-destructive">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="joining_date" id="joining_date" value="{{ old('joining_date', $driver->joining_date) }}" required
                            class="mt-1 block w-full rounded-md border-border bg-background text-foreground shadow-sm focus:border-primary focus:ring-primary">
                        @error('joining_date')
                            <p class="mt-1 text-sm text-destructive">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="blood_type" id="blood_type"
                            ...
                            @foreach(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] as $type)
                                <option value="{{ $type }}" {{ old('blood_type', $driver->blood_type) == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="medical_conditions" id="medical_conditions" rows="3"
                            class="mt-1 block w-full rounded-md border-border bg-background text-foreground shadow-sm focus:border-primary focus:ring-primary">{{ old('medical_conditions', $driver->medical_conditions) }}</textarea>
